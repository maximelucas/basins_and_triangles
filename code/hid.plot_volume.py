...
markers = ['o','s','p','h','d']
labels = [r'$\sigma=0$',r'$\sigma=1$',r'$\sigma=2$',r'$\sigma=3$',r'$\sigma=4$']
for i in range(5):
  (unique, counts) = np.unique(winding_number[i,:], return_counts=True)
  frequencies = np.asarray((unique, counts)).T
  frequencies[:,1] = frequencies[:,1]/len(winding_number[i,:])
  #print(frequencies[:,0])
  #print(frequencies[:,1])
  plt.plot(frequencies[:,0], frequencies[:,1], linestyle='--', marker=markers[i], markersize=30, color=tableau20[2*i], alpha=.9, label=labels[i])
ax.set_yscale('log')
#ax.set_xscale('log')
#ax.set_xticks([4,16,64,128])

plt.legend(loc='upper right', frameon=False, prop={'size':35}, bbox_to_anchor=(1.1, 1.05), ncol=1)
...
freq = np.loadtxt('frequencies.txt',delimiter=' ')
...
plt.xticks(fontsize = 60)
plt.yticks(fontsize = 60)
plt.xlabel(r'$\sigma$',fontsize = 70)
plt.ylabel(r'$p$',fontsize = 70)

markers = ['o','1','p','h','d','s','>','*']
labels = [r'$q=0$',r'$q=1$',r'$q=2$',r'$q=3$',r'$q=4$',r'$q=5$',r'$q=6$',r'other']
...
plt.legend(loc='upper right', frameon=False, prop={'size':35}, bbox_to_anchor=(1.2, 1.05), ncol=1)

#plt.xlim([-7,7])
plt.ylim([3*1e-4,2*1e0])
plt.gca().tick_params(axis='y', pad=25, size=15, width=3)
plt.gca().tick_params(axis='x', pad=25, size=15, width=3)
fig.set_tight_layout(True)
